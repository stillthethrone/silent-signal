...
def _dataset(tmp_path: Path, classes: int = 3) -> tuple[Path, Path]:
    ...
    del sequences["00220_front"]  # one clip without keypoints is dropped, not guessed
...
def test_training_writes_reports_and_resumes(tmp_path: Path) -> None:
    ...
    assert report["data"]["clips"] == {"train": 35, "validation": 12, "test": 12}
    assert report["data"]["dropped_without_keypoints"] == 1
