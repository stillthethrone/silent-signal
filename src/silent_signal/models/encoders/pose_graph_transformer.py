...
class PoseGraphSpatialTemporalTransformer(nn.Module):
    """Encode pose graphs with anatomical, spatial, then temporal reasoning."""
    ...
    def __init__(
        ...
        *,
        ...
        self.graph_blocks = nn.ModuleList(
            SpatialGraphBlock(hidden_dim, dropout=dropout) for _ in range(num_graph_blocks)
        )
    ...
    def forward(
        ...
        safe_spatial_valid[~safe_spatial_valid.any(dim=1), 0] = True
        spatial_tokens = self.spatial_transformer(
            spatial_tokens,
            src_key_padding_mask=~safe_spatial_valid,
        )
        spatial_tokens = spatial_tokens * spatial_valid.unsqueeze(-1).to(dtype=spatial_tokens.dtype)
        ...
        frame_tokens = frame_tokens * effective_frames.unsqueeze(-1).to(dtype=frame_tokens.dtype)
        temporal_tokens = torch.cat((self.clip_token.expand(batch, -1, -1), frame_tokens), dim=1)
