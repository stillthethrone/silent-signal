...
class FullTrainingConfig:
    ...
    coordinate_translation_jitter: float = 0.03
    ...
    def __post_init__(self) -> None:
        ...
        if min(positive) < 1:
            raise ValueError("Full-training batch, epoch, clipping, and patience must be positive.")
        if self.learning_rate <= 0 or self.weight_decay < 0:
            raise ValueError("Invalid full-training optimizer settings.")
        probabilities = (
            self.label_smoothing,
            self.coordinate_scale_jitter,
            self.coordinate_translation_jitter,
            self.joint_dropout,
        )
        if any(not 0.0 <= value < 1.0 for value in probabilities):
            raise ValueError("Training smoothing, jitter, and dropout must be in [0, 1).")
...
class GraphEncoderExperimentConfig:
    ...
    evaluation_protocol: str = "signer_disjoint"
...
def load_graph_encoder_config(path: str | Path) -> GraphEncoderExperimentConfig:
    ...
    manifest_sha256 = (
        None if raw_manifest_sha256 in (None, "", "auto") else str(raw_manifest_sha256)
    )
    ...
    return GraphEncoderExperimentConfig(
        ...
        evaluation_protocol=str(payload.get("evaluation_protocol", "signer_disjoint")),
    )
...
def model_fingerprint(config: PoseGraphModelConfig) -> str:
    ...
    if config.architecture == "graph_conv_temporal_conv_v1":
        ...
        legacy_config = {key: value for key, value in asdict(config).items() if key in legacy_keys}
        return canonical_sha256({"architecture": "pose_graph_encoder_v1", "config": legacy_config})
