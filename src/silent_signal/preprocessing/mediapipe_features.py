...
class MediaPipeFeatureConfig:
    ...
    def __post_init__(self) -> None:
        layout = get_pose_layout(self.layout_name)
        if layout.source_layout != "vsl_mediapipe_holistic_76":
            raise ValueError(
                "MediaPipeFeatureConfig requires a VSL MediaPipe Holistic 76 layout."
            )
...
def validate_raw_keypoints(raw: ArrayLike) -> NDArray[np.float32]:
    ...
    if not np.any(array != 0.0):
        raise ValueError("MediaPipe keypoints contain no observed joints.")
    return array
...
def mediapipe_graph_features(
    ...
    *,
    ...
    if not observed.any():
        raise ValueError(
            f"MediaPipe keypoints contain no observed joints in layout {config.layout_name!r}."
        )
