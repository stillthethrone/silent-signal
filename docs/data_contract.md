...
Splits allocate entire signers.

## Manifest schema

| Field | Type | Meaning |
| --- | --- | --- |
| sample_id | string | Unique physical video key: `<video_id>_<view>`. |
| instance_id | string | Synchronized VSL400 instance (the padded source video ID). |
| video_id | string | Six-digit padded source video ID. |
| signer_id | string | Three-digit padded signer ID. |
...
Older manifests may contain an empty `asl_lex_code` column from the removed
ASL Citizen adapter; readers ignore it.

## Required invariants

1. sample_id is unique.
2. Each instance has exactly one front, left and right sample.
